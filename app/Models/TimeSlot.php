...
class TimeSlot extends Model
{
    ...
    public function getAllSlots($request = null,$flag = false)
    {
        ...
        $query = self::select("time_slots.*","company.name")->whereDate('start_date_time', '>=', date('Y-m-d'))->leftJoin('company', 'company.id', '=', 'time_slots.company_id')->orderBy($column, $order);
    ...
    public function getArchievSlots($request = null,$flag = false)
    {
        $columnNumber = $request['order'][0]['column'];
        $order = $request['order'][0]['dir'];
